"""
Main script for training and evaluating the NeuralGraphGenerator model.

This script implements a two-stage generative pipeline for graphs:
1. Variational Graph Autoencoder (VGAE) based GraphTransformer model to encode and decode graphs.
2. Diffusion-based denoising model to refine latent representations and generate graphs conditionally.

The script handles:
- Command-line argument parsing for flexible model configuration.
- Preprocessing datasets into PyTorch Geometric Data objects.
- Training and validation loops for both the autoencoder and denoising model.
- Saving and loading checkpoints.
- Generating graph samples on the test set and exporting results to CSV.

Dependencies:
- PyTorch, PyTorch Geometric, NetworkX, tqdm, numpy, scipy, and related utility modules.

Usage:
Run the script from the command line with optional arguments to customize hyperparameters.
Example:
    python main.py --lr 0.001 --batch-size 128 --epochs-autoencoder 100 --train-denoiser

Author: Dheeraj Parkash
Date: 2024-12-10
"""
...
from autoencoder import GraphTransformerModel
...
# -------- Data Preprocessing --------
# Converts raw dataset into PyTorch Geometric Data objects with spectral embeddings,
# maximum node limits, and stores train/valid/test splits.
# Only once for the first time that you run the code. Then the appropriate .pt files will be saved and loaded.
...
# Data loaders for batching during training and evaluation
...
# -------- Initialize Autoencoder-GraphTransformerModel --------
...
optimizer = torch.optim.Adam(graph_transformer.parameters(), lr=args.lr)
...
if args.train_autoencoder:
    ...
    for epoch in range(1, args.epochs_autoencoder+1):
        ...
        for data in train_loader:
            data = data.to(device)
            optimizer.zero_grad()
            loss, recon, kld  = graph_transformer.loss_function(data)
        ...
        if best_val_loss >= val_loss_all:
            ...
            torch.save({
                'state_dict': graph_transformer.state_dict(),
                'optimizer' : optimizer.state_dict(),
            }, 'autoencoder.pth.tar')
else:
    checkpoint = torch.load('autoencoder.pth.tar')
    graph_transformer.load_state_dict(checkpoint['state_dict'])
...
# Initialize diffusion denoising model
...
if args.train_denoiser:
    ...
    for epoch in range(1, args.epochs_denoise+1):
        ...
        for data in train_loader:
            data = data.to(device)
            optimizer.zero_grad()
            x_g = graph_transformer.encode(data)
...
with open("output.csv", "w", newline="") as csvfile:
    ...
    for k, data in enumerate(tqdm(test_loader, desc='Processing test set',)):
        ...
        x_sample = samples[-1]
        adj = graph_transformer.decode_mu(x_sample)
